...
%% -------Define Variables-------
conditions={'FC_', 'FS_', 'FL_'}; %Name for conditions (with underscore)
...
phasestodead=3; %number of phases where fly remains in the same tile to cosider it dead
...
startoutofpoletonan=0; %Set 4,5,6,17,18 to NaN if fly starts out of pole (1), or keep values (0) 
%% ------------------------------
...
for condition=1:length(root)
    for video=1:numvideos
        ...
        x_left=diff([0; find([diff(Data(Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))<=tile_l,1)); inf]>1)]);
        x_middle=diff([0; find([diff(Data(Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))>tile_l & Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))<tile_r,1)); inf]>1)]);
        x_right=diff([0; find([diff(Data(Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))>=tile_r,1)); inf]>1)]);
        x_max=max([x_left; x_middle; x_right]);
        
        if x_max>phasestodead*fps*trialduration
...
for condition=1:length(root)
    ...
    for video=1:numvideos
        ...
        if phases(2,1)>=trialduration*fps
            phases(1,1)=phases(2,1)-trialduration*fps;
        end
        ...
        for fly=1:numflies
            ...
            for curr_phase=1:length(phases)
                ...
                if curr_phase<length(phases)
                    temp_data=Data(phases(curr_phase):phases(curr_phase+1)-1,:);
                else
                    if size(Data(phases(curr_phase):end,:),1)>=trialduration*fps
                        temp_data=Data(phases(curr_phase):phases(curr_phase)+1+trialduration*fps,:);
                    else
                ...
                if ~isempty(temp_correct) && phase(curr_phase,2)==0 && phase(curr_phase,1)~=0
                    if poletile~=2
...
% % %             close all
            ...
            %Check for dead flies and nullify if nullifydeadflies=1
            x_left=diff([0; find([diff(Data(Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))<=tile_l,1)); inf]>1)]);
            x_middle=diff([0; find([diff(Data(Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))>tile_l & Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))<tile_r,1)); inf]>1)]);
            x_right=diff([0; find([diff(Data(Data(1:phases(end,1)+1+trialduration*fps,xcolumn(1:numflies))>=tile_r,1)); inf]>1)]);
            x_max=max([x_left; x_middle; x_right]);
            
            if x_max>phasestodead*fps*trialduration && nullifydeadflies==1
                phase(:,3:end)=NaN;
            end
