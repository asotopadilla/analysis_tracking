...
```{r plot mean data}
ggplot(df_mean, aes(x=as.factor(Min), y=Speed, color=Mother, fill=Mother, group=1)) + #Change to what you want to model
...
ggplot(df, aes(x=Temp, y=Speed, color=paste0(Condition, Mother), shape=Sex)) + #Change to what you want to see
  geom_point() +
...
```{r regression}
lm <- lm(Speed ~ poly(Temp, 3) + Sex + Condition + Mother, data=df_mean)
...
ggplot(df_plot, aes(x=as.factor(Min), y=Speed, color=Mother, fill=Mother, group=1)) + #Change to what you want to see
  geom_errorbar(aes(ymin=Speed-se, ymax=Speed+se), width=0.5) +
  geom_point(size=3, shape=21, alpha=0.5) +
  scale_x_discrete(breaks=df$Min, labels=df$Temp) +
  scale_y_continuous(limits = c(0, NA)) +
